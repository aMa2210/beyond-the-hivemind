"""
Figure 4: Inter-model persona description IRCS matrix (N×N).
  Upper triangle: persona_t1 (persona, T=1) inter-model persona IRCS.
  Lower triangle: persona_t32 (persona, T=32) inter-model persona IRCS.
  Diagonal:       intra-model persona IRCS under persona_t1.

Usage:
    python -m src.pipeline.plot_fig4
"""
import numpy as np
...
from src.pipeline.fig_utils import (
    ...
    load_persona_embeddings_by_prompt,
    compute_ircs, compute_intermodel_ircs,
)

UPPER_CONFIG = "persona_t1"
LOWER_CONFIG = "persona_t32"


def compute_matrix(config, emb_root=EMB_DIR):
    """Return (N, N) matrix of pairwise inter-model persona IRCS; diagonal = intra-model."""
    n = len(MODELS)
    all_embs = {m: load_persona_embeddings_by_prompt(m, config, emb_root=emb_root) for m in MODELS}
...
def main(emb_root=EMB_DIR, fig_dir=FIG_DIR):
    ...
    print("Computing upper-triangle matrix (persona_t1)...")
    ...
    for i in range(n):
        for j in range(n):
            if i < j:
                combined[i, j] = upper[i, j]   # upper: persona_t1
            elif i > j:
                combined[i, j] = lower[i, j]   # lower: persona_t32
            else:
                combined[i, j] = upper[i, i]   # diagonal: intra-model (persona_t1)
    ...
    fig.colorbar(im, ax=ax, label="Persona Description IRCS", shrink=0.82, pad=0.01)

    ax.set_title(
        "Inter-model Persona Description IRCS\n"
        r"Upper $\triangle$: persona, T=1  ·  "
        r"Lower $\triangle$: persona, T$_{\max}$=32  ·  "
        "Diagonal: intra-model (T=1)",
        fontsize=9, pad=8,
    )
    plt.tight_layout()
    out = fig_dir / "fig4_persona_intermodel_ircs.pdf"
